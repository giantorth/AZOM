...
- **Runtime (Windows only):** `Microsoft.Win32.Registry` (in `mscorlib`) — the `MozaPortDiscovery` registry source. Under Wine/Proton that source is empty and `LinuxUsbEnumerator` reads `/sys` through Wine's unix drive instead; neither needs a package reference. The serial-probe fallback is hard-disableable via `DisableSerialProbeFallback` and is skipped entirely when the sysfs source is live.
