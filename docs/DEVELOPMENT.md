...
| `Devices/PedalHaptics/` | S12 pedal-vibration subsystem (group `0x4D`): `PedalHapticsDeviceController`, `PedalHapticsEffectWorker` (one 50 Hz thread; allocates each pedal's ShakeIt channels onto the eight interchangeable hardware slots, round-robin, sharing past that), `MozaPedalHapticsRegistry` (USB lanes by PID `0x002F` plus speculative routed lanes, and the keepalive that stops the transport idle-watchdog closing the port) |
...
| `Devices/Extensions/` | SimHub device-extension plumbing: `Moza{Wheel,Dash,Base,PedalHaptics}DeviceExtension` + their `*ExtensionSettings`, `DeviceDefinitionDeployer` (+ `.PedalHaptics` partial, which deploys one device per pedal port), `MozaDeviceExtensionFilter` |
...
- `MozaSerialConnection` — port discovery, background read/write threads (bound to a per-open generation token so a Join-timeout zombie can't re-attach to the next open — see [Threading model](#threading-model)), frame assembly, full 0x7E byte stuffing both directions, classified open-failure surface (`ConnectionFailure`: AccessDenied/PortVanished/… consumed by the UI hint builder). Two write lanes (see [Key Protocol Details](#key-protocol-details)). Registry discovery is primary; the legacy serial probe is an automatic fallback for unclassified ports only, scoped by `MozaProbeTarget` (`BaseAndHub`/`Ab9`/`HubOnly`/`PedalsOnly`/`HandbrakeOnly`/`MBooster`). "Unclassified" means *the registry cannot say who owns it* — ports it attributes to another vendor are dropped from the sweep up front via `MozaPortDiscovery.ForeignPortVids`, because probing opens the port and writes MOZA frames into it (bug reports A6N521CS / QR3760VJ: a DIY pedal among four foreign ports seized in one sweep). The per-port deadline is `ProbeJoinTimeoutMs`, **derived from** `SerialProbeCore.TotalBudgetMs` — a deadline at or below the probe's own polling budget makes the abandon path (written for the Wine freshly-powered-base wedge, which cannot close cross-thread) the normal outcome for any healthy device and leaves its port held, which is exactly what those two bundles show. Enumeration-only lanes (base-aux, hub, dashboard, standalone peripherals) hard-disable it by passing `disableProbeFallback: () => true` at construction; there is no user setting for it.
- `MozaPortDiscovery` — device enumeration behind one `PortInfo` shape (`PortName`, VID, PID, `FriendlyName`, `InstanceId`, `ContainerId`, `DevicePath`, `Serial`, `Category`), so every consumer — lane managers, the mBooster/standalone-peripheral registries, diagnostics, the Arduino-scan veto — is platform-agnostic. Two sources, picked by `WineHost.IsWine`, reported as `Source` (`Registry`/`Sysfs`/`None`) with `IsAuthoritative` = "a real source answered":
  - **Registry** (native Windows): walk of `HKLM\SYSTEM\...\Enum\USB\VID_*&PID_*`, cross-referenced against `SerialPort.GetPortNames()` to drop ghosts. The walk is deliberately vendor-agnostic and `Enumerate()` filters it to `MozaVid`; the non-MOZA remainder is what `ForeignPortVids()` reports to the probe sweep. One shared, cached pass feeds both. Lists only devices that bound `usbser`, so the serial probe can still add information and the per-port narrowing in `FindMozaPort` is unchanged.
...
- Custom WPF control library (`UI/Controls/` + `Themes/`): `SectionCard`, `SegmentedControl`, `OffOnToggle`, `PaletteStrip` (two column-aligned rows: 8 pure colours + Off/White over their 8 pastel tints + the `ColorPickerDialog` CUSTOM chip + a trailing SAVED chip mirroring `MozaPalette.SavedColor`, which `MozaPlugin.Init` seeds from `MozaPluginSettings.LastCustomLedColor` and persists through `MozaPalette.SavedColorPersist`), `KnobRingViz`, `BandwidthSparkline`, `ConnectionPill`, `MozaCurveEditor` (draggable-node output curve; `AllowHorizontalDrag` enables X-breakpoint edits — used by the wheelbase FFB output curve, where `LockLastNodeX` pins the last point at input=100 since the base has `base-ffb-curve-x1..x4` but no x5, and by the mBooster curve, which resamples its dragged X to fixed breakpoints host-side; also the FFB equalizer via style), `SteeringArc`, `TemperatureGraph`. Styles in `Themes/Generic.xaml`, tokens in `Themes/MozaTheme.xaml`, icons in `Themes/MozaIcons.xaml`.
...
**Inputs.** The saved `TelemetryDashboardKey` (`wheel:<name>` / `file:<filename>:<sha1-8>` / `builtin:<name>`) resolves to a target name, then to a target slot in the wheel's `ConfigJsonList`. All three key kinds funnel through the same slot lookup. `wheel:` keys use the entry's slot-table name (`dirName`), not its configJson `id` — the wheel reissues the id on every upload, so id-keyed bindings and channel mappings went stale each time a dashboard was re-uploaded. Pre-1.6.1 `wheel:<id>` keys still resolve (`WheelDashboardKey.Resolve`) and are rewritten in place: the profile key on first apply, the `TelemetryChannelMappings` middle keys by `ChannelMappingCoordinator.MigrateLegacyWheelKeys` (runs on configJson state-ready, on apply, and before any mapping write).
