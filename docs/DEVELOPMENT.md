...
| `DeviceTemplates/` | Embedded SimHub Device Builder `device.json` definitions, deployed lazily on first detection; `Thumbnails/` holds the per-wheel product renders deployed alongside them |
...
- **Device profile pictures** — SimHub renders a device profile's picture from a `thumbnail.png` sidecar next to `device.json`, not from a JSON field (mechanism + format spec: [`docs/simhub.md`](simhub.md#device-picture-thumbnailpng)). `DeviceTemplates/Thumbnails/<firmware-prefix>.png` holds the product renders, embedded by a csproj glob as `MozaPlugin.Devices.Thumbnails.<prefix>.png` and written out by `DeviceDefinitionDeployer.EnsureThumbnail` on every `DeployForModel` — including the definition-is-current path, so art reaches wheels whose definition was deployed by an older plugin build. It is idempotent (skips when the on-disk bytes match) and cosmetic: a thumbnail-only write never flips the "restart SimHub" result, since the picture appears at the next start regardless. Models with no art (the lookup misses) simply get none. Add a model's art by dropping `<prefix>.png` in — no code change; wheels sharing a rim get a copy per prefix (e.g. `FSR.png` = `W13.png`, the identical-looking FSR V1/V2). `Thumbnails/Unwired/` parks renders for wheels whose firmware prefix isn't known yet; it is deliberately outside the (non-recursive) glob so nothing unresolvable ships in the DLL. Regenerate with `tools/make-device-thumbnail`, which enforces the 512-long-side / 8-bit PaletteAlpha spec.
