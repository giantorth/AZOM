...
| `Devices/PedalHaptics/` | S12 pedal-vibration subsystem (group `0x4D`): `PedalHapticsDeviceController`, `PedalHapticsEffectWorker` (one 50 Hz thread holding one tone per pedal — ShakeIt mixes the effects, so there is nothing to allocate), `MozaPedalHapticsRegistry` (USB lanes by PID `0x002F` plus speculative routed lanes, and the keepalive that stops the transport idle-watchdog closing the port) |
