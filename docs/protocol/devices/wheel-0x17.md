...
**Record layout is firmware-baked — ground truth from PitHouse RE.** Decompiling `MOZA Pit House.exe` (Ghidra) resolved the `0x42` builder: 18 fixed encoder classes `FormulaSteeringTelemetryDataPackN`, where **Pack N → record type `0x0N`** (Pack size = record `PayloadLen − 5`). Each Pack is a hand-ordered field list; each field uses a typed *strategy* fixing its byte size and internal bit encoding. The wire is **LSB-first within each byte**. Transforms are `round(value)` unless noted (all capture-verified against `Tyre Data with stamps.pcapng`, an F1 2020 replay + parallel wire capture):
...
| Time (lap / gap / session) | 3 | 24-bit BE, ms | **seconds × 1000**; gap = signed delta (bit 23 = sign, unverified) |
