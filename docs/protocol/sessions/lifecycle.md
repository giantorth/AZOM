...
| Session | Opened by | Role | Description |
|---------|-----------|------|-------------|
| 0x01 | host | **Channel catalog + tier-def + string values** | Typed sub-msg framing: wheel announces per-dashboard channel catalog (`type=0x04`, URL→idx), host sends tier-defs (`type=0x01`, idx → compression/width quads), host pushes string-channel values out-of-band (`type=0x05`, ASCII), mutual seq acks (`type=0x06`). Distinct protocol from sess=0x02 — see [`session-0x01-channel-protocol.md`](session-0x01-channel-protocol.md). Also carries the display's own application log dev→host as the zlib'd UTF-16BE kind=14 payload, pulled by the host's kind=14 request and acked with kind=15 — see [`session-0x02-ff-init.md`](session-0x02-ff-init.md) § Device log pull. |
| 0x02 | host | Telemetry handshake + master catalog | FF-record protocol: init nonce / FFB property catalog (kind=11), kind=8 master channel-name catalog (PitHouse-side full list). kind=14/15 (device log pull) appear here too, but the payloads come back on sess=0x01. **Does not duplicate** the sess=0x01 tier-def — that is exclusively on sess=0x01 in current firmware. |
